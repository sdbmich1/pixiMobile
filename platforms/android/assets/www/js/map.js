var map, selectedLocation, myLocation, directionsDisplay, directionsService, mapUrl, geocoder, centerPt, markers, marker_num, locations;
...
function getMyLocation(dFlg, nearby) {
  ...
  navigator.geolocation.getCurrentPosition(function(position){ // geoSuccess
    ...
    if (nearby) {
  	  ...
  	  var parts = myLocation.toString().split(')'); 
   	  var newLoc = parts[0].split('(');
      	  newUrl = url + mapUrl + '?loc=' + newLoc[1];
      	}
    ...
    function(error){       	              
      if (!dFlg)
    	goToUrl(newUrl); // change the page
    }, geoOptions);    
...
function getCity(lat, lng) {
  ...
  mapUrl = "https://maps.googleapis.com/maps/api/geocode/json?latlng="+lat+","+lng+"&sensor=true";

  $.ajax(mapUrl).done(function(data) {
